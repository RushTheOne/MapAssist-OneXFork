    ...
    public class Overlay : IDisposable
    {
        ...
        private void _window_DrawGraphics(object sender, DrawGraphicsEventArgs e)
        {
            ...
            if (_compositor != null && _currentGameData != null)
            {
                ...
                if (time - GameManager.StartTime > TimeSpan.FromMinutes(3) && MapAssistConfiguration.Loaded.ApiConfiguration.Endpoint != BitConverter.ToString(GameManager.DefaultEndpoint) && !GameManager._valid)
                {
                    if(time.Second % 2 == 0)
                    {
                        return;
